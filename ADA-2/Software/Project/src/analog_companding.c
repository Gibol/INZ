...
uint8_t u_compress(int16_t sample, uint8_t bitLen, float uValue)
{
    ...
    value = log(1.0f+(uValue*value)) / log(1.0f+uValue);
...
int16_t u_expand(uint8_t data, uint8_t bitLen, float uValue)
{
    ...
    value = ( pow(uValue+1.0f,value) - 1.0f)/uValue;
...
int16_t A_expand(uint8_t data, uint8_t bitLen, float AValue)
{
    ...
    if(value < (1.0f / (1.0f + log(AValue) )) )
    {
        value = value * (1.0f + log(AValue) ) / AValue;
    }
    else
    {
        value = exp( (value * (1.0f + log(AValue) )) - 1) / AValue;
    }
